...
export const wandererNameTextMapHash = xavatar.find((ele) => ele.id === 10000075)!.nameTextMapHash;
...
export function getDescriptionReplaced(description: string, locale: Locale, { baseElement, baseDamage }: ReplacementDictionary = {}) {
	...
	while (ind !== -1) {
		...
		const selectors = strToReplace.substring(2, strToReplace.length-1).split('|');
		if (selectors.length > 2) console.warn(`Tcg description ${strToReplace} has extra pipes`);
		...
		switch (description[ind+2]) {
			case 'D': // D__KEY__DAMAGE or D__KEY__ELEMENT
				switch (description[ind+10]) {
					case 'D': // DAMAGE
						if (description[ind+16] !== '_') {
							if (baseDamage === undefined) {
								console.log(`Tcg object is missing skill base damage for skill`);
							}
							replacementText = baseDamage+'';
						} else {
					...
					case 'E': // ELEMENT
						const element = baseElement === 'GCG_ELEMENT_NONE' ? undefined : baseElement;
					...
					default:
						console.log(`Tcg description has unhandled replacement letter ${description[ind+2]}`);
						break;
				}
				break;
	...
	while (ind !== -1) {
		...
		else console.log(`Tcg plural has unhandled value ${number} for ${strToReplace}`);
